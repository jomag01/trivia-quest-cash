...
export default function SellerDashboard() {
  ...
  const checkSellerEligibility = async () => {
    try {
      const { data, error } = await supabase.rpc("can_become_seller", { p_user_id: user?.id });
  ...
  const checkVerificationStatus = async () => {
    try {
      const { data, error } = await supabase.from("seller_verification_requests").select("status").eq("user_id", user?.id).order("created_at", { ascending: false }).limit(1).maybeSingle();
  ...
  const handleRequestVerification = async () => {
    setSubmitting(true);
    try {
      const { error } = await supabase.from("seller_verification_requests").insert([{ user_id: user?.id }]);
      if (error) throw error;
  ...
  const handleSaveProduct = async () => {
    ...
    try {
      const productData = {
        name: productForm.name, description: productForm.description, wholesale_price: parseFloat(productForm.wholesale_price),
  ...
  return (
    <div className="container mx-auto p-4 md:p-6">
      <h1 className="text-2xl md:text-3xl font-bold flex items-center gap-2 mb-6"><Store className="h-6 md:h-8 w-6 md:w-8" />Seller Dashboard</h1>
      ...
      {profile?.is_verified_seller && (
        <Card>
          ...
          <CardContent>
            {products.map((p) => (
              <Card key={p.id} className="mb-4"><CardContent className="pt-6">
      ...
      <Dialog open={showProductDialog} onOpenChange={setShowProductDialog}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div><Label>Category</Label><Select value={productForm.category_id} onValueChange={(v) => setProductForm({ ...productForm, category_id: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{categories.map((c) => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent></Select></div>
