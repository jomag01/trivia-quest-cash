...
interface OptimizedXPostCardProps {
  post: FeedPost;
...
const OptimizedXPostCard = memo(function OptimizedXPostCard({ 
  ...
  const handleLike = useCallback(async () => {
    ...
    if (newLiked) {
      supabase.from("post_likes").insert({ post_id: post.id, user_id: user.id }).then();
    } else {
      supabase.from("post_likes").delete().eq("post_id", post.id).eq("user_id", user.id).then();
    }
  }, [user, liked, post.id]);
  ...
  const handleRepost = useCallback(async () => {
    ...
    if (newReposted) {
      supabase.from("post_shares").insert({ post_id: post.id, user_id: user.id }).then();
      toast.success("Reposted!");
    }
  }, [user, reposted, post.id]);
    ...
    if (post.media_type === "audio") {
      return (
        <div className="mt-3 p-4 rounded-2xl bg-muted/50 border border-border">
          <audio src={post.media_url} controls className="w-full" preload="none" />
        </div>
      );
    }
        ...
        <div className="flex items-center justify-between mt-3 -ml-2 max-w-md">
          ...
          >
            ...
            {(post.comments_count || 0) > 0 && (
              <span className="text-[13px]">{formatCount(post.comments_count || 0)}</span>
            )}
          </button>
          ...
          <button className="flex items-center gap-1 text-muted-foreground hover:text-primary group">
            ...
            {(post.views_count || 0) > 0 && (
              <span className="text-[13px]">{formatCount(post.views_count || 0)}</span>
            )}
          </button>
...
export default OptimizedXPostCard;
