...
import { CreateCustomAdDialog } from './CreateCustomAdDialog';
...
interface AdsPromoPopupProps {
  ...
  prefilledProduct?: ProductData;
}

const AdsPromoPopup: React.FC<AdsPromoPopupProps> = ({ open, onOpenChange, prefilledProduct }) => {
  const navigate = useNavigate();
  const [showAdDialog, setShowAdDialog] = useState(false);

  // If a product is prefilled, go directly to create ad dialog
  useEffect(() => {
    if (open && prefilledProduct) {
  ...
  // If directly promoting a product, skip promo and show ad dialog
  if (prefilledProduct) {
    return (
      <CreateCustomAdDialog 
        open={open} 
  ...
  return (
    <>
      <Dialog open={open && !showAdDialog} onOpenChange={onOpenChange}>
